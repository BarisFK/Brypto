...
<div class="container mx-auto px-4">
    <div class="fixed bottom-4 right-4 z-10">
        <button data-modal-target="addPasswordModal" data-modal-toggle="addPasswordModal" class="bg-gradient-to-r from-green-400 to-green-900 hover:from-teal-500 hover:to-teal-700 text-gray-200 font-bold py-2 px-4 rounded-full shadow-md">
            +
        </button>
    </div>

    @foreach ($passwords as $password)
    <div class="password-item bg-gradient-to-r from-green-600 to-green-400 hover:from-gray-800 hover:to-green-500 rounded-lg shadow-lg overflow-hidden mb-4 relative">
        <div class="flex flex-col md:flex-row md:items-center p-4">
            ...
            <div class="details flex-grow">
                <div class="flex items-center mb-2">
                    ...
                    <button class="copy-btn bi bi-clipboard bg-teal-600 text-gray-200 py-1 px-2 rounded-md hover:bg-teal-700 focus:outline-none" data-clipboard-target="#username-{{ $password->id }}"></button>
                    <input type="hidden" id="username-{{ $password->id }}" value="{{ $password->username }}">
                </div>
                <div class="flex items-center mb-2">
                    <p class="text-gray-200 mr-4 flex-grow"><strong>Password:</strong> <span id="password-{{ $password->id }}">{{ $password->password }}</span></p>
                    <button class="copy-btn bi bi-clipboard bg-teal-600 text-gray-200 py-1 px-2 rounded-md hover:bg-teal-700 focus:outline-none" data-clipboard-target="#password-{{ $password->id }}"></button>
                </div>
        ...
        <div class="delete-button absolute bottom-2 right-2">
            <form action="{{ route('passAdd', $password->id) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" class="bi bi-trash bg-red-500 hover:bg-red-600 text-gray-200 py-1 px-2 rounded-md focus:outline-none">
...
<div id="addPasswordModal" class="fixed z-50 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg px-8 py-6 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            ...
            <form action="{{ route('passAdd') }}" method="POST">
                ...
                <div class="mt-6">
                    <button type="submit" class="w-full bg-green-500 hover:bg-green-700 text-gray-200 font-bold py-2 px-4 rounded">
                        Add Password
                    </button>
                </div>
            </form>
